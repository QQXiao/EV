...
for sub=1:length(subs)
  ...
  for r=1:4 % run
        ...
        for mem=[1:3] % R,K,F  
            a=find(trial_list_all(:,Mwm)==mem); % trial index
            ...
            incld_idx=setdiff(all_idx,[same_rep within_idx]);
            %incld_idx=setdiff(all_idx,[same_rep diff_cond within_idx]);
          ...
          for k=1:length(within_dis)
              ...
              kk=1;
              while isempty(tmp) & kk<20 % find closer ones
                  tmp=find((all_dis==(within_dis(k)-kk) | all_dis==(within_dis(k)+kk)) & all_tsk==within_tsk(k));
                  kk=kk+1;
              end
              
              if isempty(tmp)
                  bad_trial=[bad_trial;new_label(k,:)];
                  between_idx=[between_idx 1];
                  between_dis=[between_dis 1];
...
%           
          ...
          for roi=1:length(roi_name);
              ...
              %xx=tmp_raw(4:end,1:end); % remove the final zero and the first three rows showing the coordinates
              xx=tmp_raw(4:end,1:end-1); % remove the final zero and the first three rows showing the coordinates
              
              % whether or not to remove voxels showing difference in ACT
              H=ttest2(xx(trial_list_all(:,Mwm)==1,:),xx(trial_list_all(:,Mwm)==3,:),0.05,'Both','unequal');                       
...
%                   yy(i,voxel_idx)=yy(i,:)./max(yy(i,voxel_idx));
%               end                   
%               
              %cc=1-pdist(xx(:,voxel_idx),'correlation'); % use xx or yy to switch from normalize to no-normalize
    ...
    eval(sprintf('save %s/LSA/sme/all/sme_sub%02d_RSA_ROI all_act_within all_act_between all_RSA_within all_RSA_between all_dis_within all_dis_between new_label', datadir,subs(sub)));
    %eval(sprintf('save %s/LSS/sme/no/sme_sub%02d_RSA_ROI all_act_within all_act_between all_RSA_within all_RSA_between all_dis_within all_dis_between new_label', datadir,subs(sub)));
