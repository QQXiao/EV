...
for sub=1:length(subs)
    ...
    load(sprintf('%s/LSA/sme/%s/sme_sub%02d_RSA_ROI.mat',datadir,dir,subs(sub)));    
...
%              all_RSA_between(:,r)=c.resid;
            ...
            for i=1:2           
              mem=mem_cond(i);  
                tmp=mean(all_RSA_within(new_label(:,Mwm)==mem,r));
                tmp1=mean(all_RSA_between(new_label(:,Mwm)==mem,r));
                
                all_RSA(r,sub,i)=tmp;
                all_RSA(r,sub,i+2)=tmp1;
            end
...
%     
    ...
    condnames={'Remembered','Forgottern'};
