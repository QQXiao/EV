...
act.null <- lmer(act~1+(1|subid)+(1|wid),REML=FALSE,data=titem_data)
act <- lmer(act~lag+(1|subid)+(1|wid),REML=FALSE,data=titem_data)
...
ps.null <- lmer(ps~1+(1|subid)+(1|wid),REML=FALSE,data=titem_data)
ps <- lmer(ps~lag+(1|subid)+(1|wid),REML=FALSE,data=titem_data)
...
write.matrix(r.itemInfo,file=paste("nidx_lag.txt",sep=""),sep="\t")
