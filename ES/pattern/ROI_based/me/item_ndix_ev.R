...
act <- lmer(act~ev+(1|subid)+(1|wid),REML=FALSE,data=titem_data)
...
ps <- lmer(ps~ev+(1|subid)+(1|wid),REML=FALSE,data=titem_data)
...
write.matrix(r.itemInfo,file=paste("nidx_ev.txt",sep=""),sep="\t")
