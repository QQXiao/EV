...
act.null <- glmer(mem~1+(act|subid)+(act|wid),data=titem_data,family='binomial')
act <- glmer(mem~act+(act|subid)+(act|wid),data=titem_data,family='binomial')
...
ps.null <- glmer(mem~1+(ps|subid)+(ps|wid),data=titem_data,family='binomial')
ps <- glmer(mem~ps+(ps|subid)+(ps|wid),data=titem_data,family='binomial')
...
write.matrix(r.itemInfo,file=paste("mem_nidx.txt",sep=""),sep="\t")
