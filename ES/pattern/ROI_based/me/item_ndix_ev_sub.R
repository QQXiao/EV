...
act.null <- lmer(act~1+(1|subid)+(1|wid),REML=FALSE,data=mdata)
act <- lmer(act~ev+(1|subid)+(1|wid),REML=FALSE,data=mdata)
...
ps.null <- lmer(ps~1+(1|subid)+(1|wid),REML=FALSE,data=mdata)
ps <- lmer(ps~ev+(1|subid)+(1|wid),REML=FALSE,data=mdata)
...
act.null <- lmer(act~1+(1|subid)+(1|wid),REML=FALSE,data=sdata)
act <- lmer(act~ev+(1|subid)+(1|wid),REML=FALSE,data=sdata)
mainEffect.lag <- anova(act,act.null)
...
ps.null <- lmer(ps~1+(1|subid)+(1|wid),REML=FALSE,data=sdata)
ps <- lmer(ps~ev+(1|subid)+(1|wid),REML=FALSE,data=sdata)
mainEffect.lag <- anova(ps,ps.null)
...
write.matrix(r.itemInfo,file=paste("nidx_ev_sub.txt",sep=""),sep="\t")
