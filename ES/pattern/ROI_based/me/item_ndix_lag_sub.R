...
r.itemInfo <- matrix(data=NA, nr=length(roi_name), nc=19)
...
idata <- subset(titem_data,ev==2)

act.null <- lmer(act~1+(1|subid)+(1|wid),REML=FALSE,data=cdata)
act <- lmer(act~lag+(1|subid)+(1|wid),REML=FALSE,data=cdata)
...
ps.null <- lmer(ps~1+(1|subid)+(1|wid),REML=FALSE,data=cdata)
ps <- lmer(ps~lag+(1|subid)+(1|wid),REML=FALSE,data=cdata)
...
act.null <- lmer(act~1+(1|subid)+(1|wid),REML=FALSE,data=idata)
act <- lmer(act~lag+(1|subid)+(1|wid),REML=FALSE,data=idata)
mainEffect.lag <- anova(act,act.null)
r.itemInfo[i,11]=mainEffect.lag[2,6]
r.itemInfo[i,12]=mainEffect.lag[2,7]
r.itemInfo[i,13]=mainEffect.lag[2,8]
r.itemInfo[i,14]=fixef(act)[2];

ps.null <- lmer(ps~1+(1|subid)+(1|wid),REML=FALSE,data=idata)
ps <- lmer(ps~lag+(1|subid)+(1|wid),REML=FALSE,data=idata)
mainEffect.lag <- anova(ps,ps.null)
r.itemInfo[i,16]=mainEffect.lag[2,6]
r.itemInfo[i,17]=mainEffect.lag[2,7]
r.itemInfo[i,18]=mainEffect.lag[2,8]
r.itemInfo[i,19]=fixef(ps)[2];
}
write.matrix(r.itemInfo,file=paste("nidx_lag_sub.txt",sep=""),sep="\t")
